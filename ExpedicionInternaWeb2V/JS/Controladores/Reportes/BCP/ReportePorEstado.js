angular.module('simihApp').controller('ReportePorEstadoController',
    function ($scope, localStorageService, $filter, $rootScope, Requester, $modal, uiGridConstants, $alert, ExportarService) {
        ...
        $scope.estadosList = [
            { estadoId: 0, estadoNombre: 'Todos' },
            { estadoId: 1, estadoNombre: 'Creado' },
            { estadoId: 2, estadoNombre: 'Custodia' },
            { estadoId: 3, estadoNombre: 'Ruta' },
            { estadoId: 4, estadoNombre: 'Recibido' },
            { estadoId: 5, estadoNombre: 'Confirmado' },
            { estadoId: 6, estadoNombre: 'Retirado' }
        ];
        ...
        $scope.gridReportePorEstado.onRegisterApi = function (gridApi) {
        ...
        $scope.gridReportePorEstado.data = [];

        $scope.generarReportePorEstado = () => {
                

            Requester.AuthorizationPost($rootScope.rutaWebGeneral + 'ReportePorEstado', {
                "estadoId": $scope.estadoSeleccionado,
                "fechaInicio": $scope.fromDate,
                "fechaFin": $scope.untilDate
            }).then(function (data) {
                $scope.gridReportePorEstado.data = data;
            }, function (status) {
                $alert(
                    {
                        title: 'ALERTA!',
        ...
        $scope.exportarReporte = () => {
            ...
            ObjetoExportar.NameReporte = "ReportePorEstado";
            var obj = [];
            var datosExportar = [];
            angular.copy($scope.gridReportePorEstado.data, datosExportar);
        ...
        $scope.SelectEstadoIndexChange = function () {
            $scope.estadoSeleccionado = $scope.estado.estadoId;            
        }
