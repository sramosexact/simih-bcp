...
app.controller('ModificarRecepcionLimaCtrl', ['$scope', '$rootScope', '$http', 'ServiciosApi', '$uibModalInstance', '$filter',
    function ($scope, $rootScope, $http, ServiciosApi, $uibModalInstance, $filter) {
        ...
        if ($scope.AgenciaSeleccionada.iIdEstado === 1) {
            $scope.tituloModificacion = 'Registrar datos del traslado';
            $scope.nombreBotonModificar = 'Registrar';
        }
       
        if ($scope.isUndefinedOrNullOrEmpty($scope.AgenciaSeleccionada.dFechaRegistro)) {
            $scope.fechaRegistro = new Date($scope.AgenciaSeleccionada.dFecha);
            $scope.horasVisita = '';
            $scope.minutosVisita = '';
        ...
        if ($scope.isUndefinedOrNullOrEmpty($scope.AgenciaSeleccionada.dFechaRecepcion)) {
            $scope.fechaRecepcion = new Date($scope.AgenciaSeleccionada.dFecha);
        ...
        $scope.guardarCambios = function () {
            ...
            if (dRegistro > fechaActual) {
                swal("\u00a1Error!", "La hora de recojo no puede ser mayor que la actual.", "error");
                return;
            }
            if (horaRecepcionVacio) {
                swal("\u00a1Error!", "Debes ingresar la hora de recepción", "error");
                return;
            }
            ...
            if (dRecepcion > fechaActual) {
                swal("\u00a1Error!", "La hora de recepci\u00f3n no puede ser mayor que la actual.", "error");
                return;
            }
            if (dRegistro > dRecepcion) {
                swal("\u00a1Error!", "La hora de visita no puede ser mayor que la de recepci\u00f3n de las valijas.", "error");
                return;
            }                
