app.controller('ReporteRecepcionAgenciaProvinciaCtrl', ['$scope', '$rootScope', '$http', 'ServiciosApi', '$location', '$timeout', 'Exportar', '$filter', 'uiGridConstants', 'NgTableParams', 'TIPO',
    function ($scope, $rootScope, $http, ServiciosApi, $location, $timeout, Exportar, $filter, uiGridConstants, NgTableParams, TIPO) {
        ...
        *******************************/
        $scope.ObtenerAgenciaReportadas = function () {
            ...
            ServiciosApi.getGenerarReporteRecepcionProvincia(strDesde, strHasta, 0).then(function (data) {
                if (data.length === 0) {
                ...
                angular.forEach(data, function (obj, index) {
                    $scope.Elemento = {
                        ...
                        "ArmadoChart": [
                            ...
                            //},
                            {
                                value: obj.RecogidosATiempo,
                                color: "#247EBA",
                                highlight: "#2EA1ED",
                                label: "Dentro del tiempo estimado"
                            },
                            {
                                value: obj.RecogidosFueraRango,
                                color: "#F29A12",
                                highlight: "#F2CC12",
                                label: "Fuera del tiempo estimado"
                            }
        ...
        *************************************/
        ...
        var setCamposEstaticos = function () {
            CampoGridList = [
                ...
                { displayName: 'Plazo', field: 'iHorasPlazoRecepcion', width: '50', enableColumnResizing: true, filter: { condition: uiGridConstants.filter.CONTAINS }, headerCellClass: 'header-grid-color', cellClass: 'row-style' },
                { displayName: 'Fecha planeada de llegada a UTD', field: 'dFechaTentativaRecepcion', width: '80', enableColumnResizing: true, filter: { condition: uiGridConstants.filter.CONTAINS }, type: 'date', cellFilter: 'date: "dd/MM/yyyy"', headerCellClass: 'header-grid-color', cellClass: 'row-style' },
        ...
        var getCamposDinamicos = function () {
            ...
            ServiciosApi.listarCamposDinamicosReporte(tipoRegistro, tipoAgencia).then(function (data) {
                ...
                console.log($rootScope.camposDinamicos);
                CampoGridList = [];
                ...
                CampoGridList.push({
                    name: 'Ind. Recojo', displayName: 'Ind. Recojo', width: '80', enableFiltering: false, filter: { condition: uiGridConstants.filter.CONTAINS }, cellTemplate: tmpIndicador, headerCellClass: 'header-grid-color',
                    cellClass: function (grid, row, col, rowRenderIndex, colRenderIndex) {
                        return $scope.getColorIndicador(row.entity) + ' row-style';
                    }
                });
                CampoGridList.push({
                    name: 'Ind. Recepción', displayName: 'Ind. Recepción', width: '80', enableFiltering: false, filter: { condition: uiGridConstants.filter.CONTAINS }, cellTemplate: tmpIndicadorRecepcion, headerCellClass: 'header-grid-color',
                    cellClass: function (grid, row, col, rowRenderIndex, colRenderIndex) {
                        return $scope.EvaluarIndicadorClase(row.entity.iIdIndicadorRecepcion) + ' row-style';
                    }
                });
        ...
        $scope.getObtenerDetalleRecepcion = function () {
            ...
            ServiciosApi.getGenerarReporteRecepcionDetalleProvincia(pFechaDesde, pFechaHasta).then(function (data) {
        ...
        $scope.ExportarTablaDetalle = function () {
            ...
            ObjetoExportar.NameReporte = "RecepcionValijaProvincia";
            var obj = [];
            var datosExportar = [];
            ...
            angular.forEach(datosExportar, function (value, index) {
                ...
                obj.push(EvaluarIndicadorProvincia(value.iIdIndicadorRecepcion));
